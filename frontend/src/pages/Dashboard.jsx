...
const CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .fab-btn {
    display: flex; align-items: center; gap: 10px; padding: 12px 20px; border-radius: 12px; border: none;
    font-family: 'Syne', sans-serif; font-size: 13px; font-weight: 700; cursor: pointer; transition: all 0.2s;
  }
  ...
  .risk-legend {
    ...
    background: rgba(11,17,32,0.9); border: 1px solid rgba(255,255,255,0.08);
  ...
  .leaflet-popup-content-wrapper {
    background: #0f1f35 !important; border: 1px solid rgba(0,200,255,0.2) !important;
    border-radius: 12px !important; color: #fff !important; box-shadow: 0 8px 32px rgba(0,0,0,0.5) !important;
  }
...
const NAV = [
  { icon: "◉", label: "Live Map",      roles: ["PUBLIC","RESCUE","ADMIN"], active: true  },
  { icon: "⚡", label: "Predictions",   roles: ["PUBLIC","RESCUE","ADMIN"], active: false },
  { icon: "🤖", label: "AI Chat",       roles: ["RESCUE","ADMIN"],          active: false },
  { icon: "🔔", label: "Alerts",        roles: ["RESCUE","ADMIN"],          active: false },
  { icon: "⬡",  label: "Manage Zones",  roles: ["ADMIN"],                   active: false },
  { icon: "👥", label: "Users",         roles: ["ADMIN"],                   active: false },
];

export default function Dashboard() {
  const [zones,       setZones]       = useState([]);
  const [predictions, setPredictions] = useState({}); // zone_id → {flood_risk, fire_risk, overall_risk, ...}
  ...
  useEffect(() => {
    ...
    // Load latest predictions for map coloring
          ...
          <div className="map-wrap" ref={wrapRef}>
                    ...
                    >
                      <Popup>
                        ...
                        <div className="popup-row">
                          ...
                          <span className={`popup-risk popup-risk-${risk}`}>{risk}</span>
                        </div>
                        <div className="popup-row">
                          <span>Flood</span>
                          <span className="popup-val">
                            {pred ? `${pred.flood_risk} (${Math.round(pred.flood_prob * 100)}%)` : "—"}
                          </span>
                        </div>
                        <div className="popup-row">
                          <span>Fire</span>
                          <span className="popup-val">
                            {pred ? `${pred.fire_risk} (${Math.round(pred.fire_prob * 100)}%)` : "—"}
                          </span>
                        </div>
                        ...
                        {pred?.last_updated && (
                          <div className="popup-row">
                            <span>Updated</span>
                            <span className="popup-val">{new Date(pred.last_updated).toLocaleTimeString()}</span>
                          </div>
                        )}
                      </Popup>
                    </Marker>
            ...
            {role === "ADMIN" && (
              <div className="admin-fab">
                <button className="fab-btn fab-primary">⬡ Add Zone</button>
              </div>
            )}
          </div>
        </div>
